...
public class GrafoHM{
    ...
    //Ejercicio 2
    public HashMap<String, LinkedList<Nodo>> eliminarVertice(String vertice){
        LinkedList<Nodo> vecinos = grafo.get(vertice);
        LinkedList<String> nomVec = new LinkedList<>();
        for(Nodo i: vecinos){
            nomVec.add(i.getVertice());
        }
        grafo.remove(vertice);
        for(String j: nomVec){
            eliminarArista(j, vertice);
        }
        return grafo;
    }
    private void eliminarArista(String origen,String destino){
        LinkedList<Nodo> listaVec = grafo.get(origen);
        boolean bandera = false;
        for(int i=0;i<listaVec.size() && bandera==false;i++){
            Nodo ac = listaVec.get(i);
            if(ac.getVertice().equals(destino)){
                listaVec.remove(i);
                bandera = true;
            }
        }
